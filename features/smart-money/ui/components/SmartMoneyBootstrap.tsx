...
export default function SmartMoneyBootstrap() {
  const [collecting, setCollecting] = useState(false);
  const triggered = useRef(false);
  const setRefreshKey = useSetAtom(investorFlowRefreshAtom);

  useEffect(() => {
    if (triggered.current) return;
    triggered.current = true;

    checkInvestorFlowHasData().then((hasData) => {
      if (hasData) return;

      setCollecting(true);
      triggerCollectInvestorFlow()
        .then(() => {
          setRefreshKey((k) => k + 1);
        })
        .catch(() => {
          // 수집 실패 시 조용히 무시 — 사용자는 빈 화면 확인
        })
        .finally(() => {
          setCollecting(false);
        });
  ...
  return (
    <div className="mb-4 flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700 dark:border-blue-800 dark:bg-blue-950/40 dark:text-blue-300">
      ...
      KRX 순매수 데이터를 처음 수집하고 있습니다. 잠시 기다려 주세요...
    </div>
  );
}
